---
name: surf
description: Use when the user needs browser automation, screenshots, form filling, page inspection, network/CPU emulation, DevTools streaming, or AI queries via ChatGPT/Gemini/Claude/AI Mode/Perplexity/Grok/AI Studio. No API keys required - uses your browser's logged-in sessions.
---

# Surf Browser Automation (v2.7.1)

Control Chrome browser via CLI for testing, automation, and debugging. No API keys required - uses your browser's logged-in sessions.

## CLI Quick Reference

```bash
surf --help                    # Full help
surf <command> --help          # Command-specific help
surf --version                 # Show version
```
...
**CRITICAL:** All `surf` commands MUST be executed sequentially. Because of the Chrome extension's native messaging bottleneck (one connection at a time), parallel `surf` calls will queue up and timeout.

### ChatGPT
```bash
surf chatgpt "explain quantum computing"
surf chatgpt "summarize this page" --with-page   # Include page context
surf chatgpt "review code" --file code.ts         # Attach file
surf chatgpt "analyze" --model gpt-5.4             # Specify model
surf chatgpt "help me debug" --timeout 2700       # Custom timeout (default: 2700s = 45min)
```

Query ChatGPT at `chatgpt.com` using your browser's logged-in session.

### AI Mode
```bash
surf aimode "explain this code"
surf aimode "summarize" --with-page              # Include current page context
surf aimode "question" --pro                     # Pro mode (nem=143) — higher quality
surf aimode "complex query" --timeout 300        # Custom timeout (default: 120s)
```
...
surf gemini "hello" --model gemini-flash-latest   # Specify model
...
surf aistudio "quick answer" --model gemini-3.1-pro-preview  # Model selection (⚠️ gemini-3-pro-preview deprecated)
...
**Model selection is best-effort:** Pass any AI Studio model id (e.g. `gemini-3.1-pro-preview`, `gemini-3-flash-preview`, `gemini-2.5-pro`). If the model isn't found, AI Studio uses whatever model was last selected in the UI.

**⚠️ Migration:** `gemini-3-pro-preview` was shut down March 9, 2026. Migrate to `gemini-3.1-pro-preview`.

### AI Studio App Builder
```bash
surf aistudio.build "build a portfolio site"
surf aistudio.build "todo app" --model gemini-3.1-pro-preview   # Model override (current flagship)
...
### AI (requires GOOGLE_API_KEY)
```bash
surf ai "find the login button"          # Mode auto-detected (find)
surf ai "summarize this page"            # Mode auto-detected (summary)
surf ai "extract all links as json" --mode extract  # Explicit mode
```

Query AI using Google API key directly. Requires `GOOGLE_API_KEY` environment variable.

Modes (auto-detected from query): `find` | `summary` | `extract`

### Claude (via claude.ai - requires Claude AI login in Chrome)
```bash
surf claude "explain quantum computing"
surf claude "summarize" --with-page               # Include page context
surf claude "review code" --model claude-sonnet-4-6  # Specify model
surf claude "help me debug" --timeout 300         # Custom timeout (default: 300s)
```

Query Claude AI at `claude.ai` using your browser's logged-in session. Claude is particularly strong at reasoning, code analysis, and thoughtful responses.

### Google AI Mode (via gemini.google.com - requires Google login in Chrome)
```bash
surf aimode "what is quantum computing"           # Auto mode (udm=50)
surf aimode "explain this page" --with-page       # Include page context
surf aimode "question" --pro                      # Pro mode (nem=143) — higher quality
surf aimode "complex query" --timeout 300         # Custom timeout (default: 120s)
```

Query Google AI Mode at `gemini.google.com` (use `aimode` as the command). AI Mode is Google's AI-powered search mode in Gemini, offering real-time web access with AI reasoning.

**AI Mode Notes:**
- Default mode uses `udm=50` (auto). Use `--pro` to switch to `nem=143` (pro mode) for better research quality.
- Best for questions requiring current information or web search
...
1. **Not logged in**: The error "login required" means you need to log into the service in Chrome (chatgpt.com, claude.ai, gemini.google.com, perplexity.ai, x.com, or aistudio.google.com)
...
# Smart type - types into specific element using JS method
surf smart_type --ref e5 --text "hello"
...
```bash
surf batch                            # Execute multiple actions from JSON stdin
```

Execute multiple actions in sequence from JSON input.
...
```bash
surf scroll                    # Scroll in direction
surf scroll.bottom
surf scroll.top
...
surf network.path --id "req-123"  # Get file paths for request data
```
...
## Agentic Online Research Workflow

If you (the AI agent) need to perform comprehensive online research to answer a complex user request or gather the latest facts:
1. Open a terminal and use `run_command` with the following syntax:
   `surf aimode "your comprehensive query here"`
2. Wait for the browser session to complete its research and review the results printed to the terminal.
3. Incorporate the findings into your analysis or response.

> **CRITICAL RULE - SEQUENTIAL EXECUTION ONLY:** Do NOT run multiple `surf` commands in parallel. The Chrome extension's native messaging only supports ONE connection at a time. All `surf` requests MUST be executed sequentially (wait for one to finish before starting the next). Parallel requests will queue up and cause a timeout.

*Note: This relies on the user's AI Mode logged-in session, ensuring high-quality research outcomes without needing API keys.*
...
--auto-capture        # Screenshot + console on error (saves to /tmp)
--soft-fail           # Warn and exit 0 on error (for non-critical commands)
...
surf --script <file> --dry-run    # Validate without executing
```
...
6. **AI tools use browser session** - Must be logged into the service (ChatGPT, Claude, Gemini, AI Mode, Perplexity, Grok, AI Studio), no API keys needed
...
15. **Soft-fail for non-critical checks** - Use `--soft-fail` to warn and exit 0 on errors
16. **Script mode for automation** - `surf --script workflow.json --dry-run` validates before running
